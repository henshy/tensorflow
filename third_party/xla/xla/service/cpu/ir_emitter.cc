...
class IrEmitter::CpuElementalIrEmitter : public ElementalIrEmitter {
 public:
  CpuElementalIrEmitter(const HloModuleConfig& module_config,
      ...
      : ElementalIrEmitter(module, ir_emitter->builder()),
 ...
 protected:
  absl::StatusOr<llvm::Value*> EmitAtan2(PrimitiveType prim_type,
    ...
    return xla::cpu::EmitAtan2(module(), *builder(), prim_type, lhs, rhs);
  }

  absl::StatusOr<llvm::Value*> EmitTanh(PrimitiveType prim_type,
                                        llvm::Value* value) override {
    return xla::cpu::EmitTanh(module(), *builder(), prim_type, value);
  }

  absl::StatusOr<llvm::Value*> EmitErf(PrimitiveType prim_type,
                                       llvm::Value* value) override {
    return xla::cpu::EmitErf(module(), *builder(), prim_type, value);
  }
...
IrEmitter::IrEmitter(mlir::MLIRContext* mlir_context,
  ...
  builder()->setFastMathFlags(llvm_ir::GetCpuFastMathFlags(hlo_module_config_));
  ...
  if (ShapeUtil::IsScalar(return_shape)) {
    ...
    for (int i = 0; i < return_shape.tuple_shapes_size(); i++) {
      ...
      llvm::Value* source = llvm_ir::EmitGetTupleElement(
          ...
          root_value.GetBasePointer(), root_value.GetBasePointeeType(),
          builder());
...
void IrEmitter::InitializeIrFunction(const std::string& function_name) {
  ...
  compute_function_ = std::make_unique<IrFunction>(
      function_name, linkage, hlo_module_config_, module_, builder(),
      num_dynamic_loop_bounds_);
}
...
absl::Status IrEmitter::HandleGetTupleElement(
  ...
  emitted_value_[get_tuple_element] = llvm_ir::EmitGetTupleElement(
      ...
      GetEmittedValueFor(operand), IrShapeType(operand->shape()), builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleInfeed(HloInstruction* instruction) {
  ...
  if (data_shape.IsTuple()) {
    ...
    llvm_ir::EmitTuple(llvm_ir::IrArray(data_address, data_type, data_shape),
                       tuple_element_addresses, builder());
  } else {
    TF_RETURN_IF_ERROR(
...
absl::Status IrEmitter::EmitXfeedTransfer(XfeedKind kind, const Shape& shape,
  ...
  llvm::Value* acquired_pointer = EmitCallToFunc(
      acquire_func_name,
      {GetExecutableRunOptionsArgument(), builder()->getInt32(length_32),
       shape_ptr, builder()->getInt32(shape_length)},
      builder()->getPtrTy());
  if (kind == XfeedKind::kInfeed) {
    ...
    if (emit_code_for_msan_) {
      ...
      llvm::Type* intptr_type = builder()->getIntPtrTy(dl);
      EmitCallToFunc(
          "__msan_unpoison",
  ...
  EmitCallToFunc(
      release_func_name,
      {GetExecutableRunOptionsArgument(), builder()->getInt32(length_32),
       acquired_pointer, shape_ptr, builder()->getInt32(shape_length)},
      builder()->getVoidTy());
...
absl::Status IrEmitter::HandleOutfeed(HloInstruction* outfeed) {
  ...
  for (int i = 0; i < operand_shape.tuple_shapes_size(); ++i) {
    ...
    llvm::Value* tuple_element = llvm_ir::EmitGetTupleElement(
        ...
        value, IrShapeType(operand_shape), builder());
...
absl::Status IrEmitter::HandleSort(HloInstruction* hlo) {
  ...
  llvm::Value* values = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
      builder()->getPtrTy(), builder()->getInt32(sort->operand_count()),
      "cc_values_alloca", builder());
  llvm::Value* sizes = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
      builder()->getInt32Ty(), builder()->getInt32(sort->operand_count()),
      "cc_sizes_alloca", builder());
  for (int64_t i = 0; i < sort->operand_count(); ++i) {
    llvm::Value* slot_in_values_alloca =
        ConstInBoundsGEP1_32(builder()->getPtrTy(), values, i);
    Store(destination_addresses[i], slot_in_values_alloca);
    llvm::Value* slot_in_sizes_alloca =
        ConstInBoundsGEP1_32(builder()->getInt32Ty(), sizes, i);
    llvm::Value* size = builder()->getInt32(ShapeUtil::ByteSizeOfPrimitiveType(
  ...
  EmitCallToFunc(
      ...
      {builder()->getInt64(higher_dimensions),
       builder()->getInt64(sort_dimension_elements),
       builder()->getInt64(lower_dimensions), values,
       builder()->getInt32(sort->operand_count()), sizes,
       builder()->getInt1(sort->is_stable()), GetExecutableRunOptionsArgument(),
       GetProfileCountersArgument(), less_than_function},
      builder()->getVoidTy());

  if (sort->values_count() > 0) {
    llvm_ir::EmitTuple(GetIrArrayFor(sort), destination_addresses, builder());
  }
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleTuple(HloInstruction* tuple) {
  ...
  llvm_ir::EmitTuple(GetIrArrayFor(tuple), base_ptrs, builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleSelectAndScatter(
  ...
  llvm_ir::ForLoopNest source_loops(IrName(select_and_scatter), builder());
  const llvm_ir::IrArray::Index source_index =
      source_loops.AddLoopsForShape(source->shape(), "source");
  SetToFirstInsertPoint(source_loops.GetInnerLoopBodyBasicBlock(), builder());
  ...
  llvm::AllocaInst* selected_value_address = llvm_ir::EmitAllocaAtFunctionEntry(
      ...
      "selected_value_address", builder(),
      MinimumAlignmentForPrimitiveType(operand_element_type));
  llvm::AllocaInst* selected_index_address =
      llvm_ir::EmitAllocaAtFunctionEntryWithCount(
          builder()->getInt64Ty(), builder()->getInt32(rank),
          "selected_index_address", builder());
  llvm::AllocaInst* initialized_flag_address =
      llvm_ir::EmitAllocaAtFunctionEntry(builder()->getInt1Ty(),
                                         "initialized_flag_address", builder());
  Store(builder()->getInt1(false), initialized_flag_address);
  ...
  SetToFirstInsertPoint(window_loops.GetInnerLoopBodyBasicBlock(), builder());

  // Compute the operand index to visit and evaluate the condition whether the
  // operand index is within the bounds. The unsigned comparison includes
  // checking whether the operand index >= 0.
  llvm::SmallVector<llvm::Value*> operand_multi_index(source_index.size());
  llvm::Value* in_bounds_condition = builder()->getTrue();
  for (int64_t i = 0; i < rank; ++i) {
    llvm::Value* strided_index = NSWMul(
        source_index[i], builder()->getInt64(window.dimensions(i).stride()));
    operand_multi_index[i] =
        NSWSub(NSWAdd(strided_index, window_index[i]),
               builder()->getInt64(window.dimensions(i).padding_low()));
    llvm::Value* index_condition = ICmpULT(
        operand_multi_index[i],
        builder()->getInt64(ShapeUtil::GetDimension(operand->shape(), i)));
  ...
  llvm_ir::LlvmIfData if_in_bounds =
      llvm_ir::EmitIfThenElse(in_bounds_condition, "in-bounds", builder());
  SetToFirstInsertPoint(if_in_bounds.true_block, builder());
  ...
  SetToFirstInsertPoint(if_initialized.false_block, builder());
        ...
        for (int64_t i = 0; i < rank; ++i) {
          llvm::Value* selected_index_address_slot =
              InBoundsGEP(selected_index_address->getAllocatedType(),
                          selected_index_address, {builder()->getInt32(i)});
          Store(operand_index[i], selected_index_address_slot);
        }
      };
  llvm_ir::IrArray::Index operand_index(
      operand_multi_index, operand_array.GetShape(), builder()->getInt64Ty());
  llvm::Value* operand_data =
      operand_array.EmitReadArrayElement(operand_index, builder());
  Store(operand_data, selected_value_address);
  save_operand_index(operand_index);
  Store(builder()->getInt1(true), initialized_flag_address);
  ...
  SetToFirstInsertPoint(if_initialized.true_block, builder());
  llvm::Value* operand_address =
      operand_array.EmitArrayElementAddress(operand_index, builder());
  ...
  llvm_ir::LlvmIfData if_select_lhs =
      llvm_ir::EmitIfThenElse(cond, "if-select-lhs", builder());
  SetToFirstInsertPoint(if_select_lhs.false_block, builder());
  ...
  SetToFirstInsertPoint(window_loops.GetOuterLoopExitBasicBlock(), builder());
  llvm::SmallVector<llvm::Value*> selected_multi_index;
  for (int64_t i = 0; i < rank; ++i) {
    const std::vector<llvm::Value*> gep_index = {builder()->getInt32(i)};
    llvm::Value* selected_index_address_slot =
        InBoundsGEP(selected_index_address->getAllocatedType(),
  ...
  llvm::Value* source_value =
      source_array.EmitReadArrayElement(source_index, builder());
  ...
  llvm::Value* output_value =
      output_array.EmitReadArrayElement(selected_index, builder());
  llvm::Value* scatter_value = EmitScalarReturningThreadLocalCall(
      *select_and_scatter->scatter(), {output_value, source_value},
      "scatter_function");
  output_array.EmitWriteArrayElement(selected_index, scatter_value, builder());

  SetToFirstInsertPoint(source_loops.GetOuterLoopExitBasicBlock(), builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleDot(HloInstruction* dot) {
  ...
  return EmitDotOperation(*dot, target_array, lhs_array, rhs_array,
                          ...
                          GetExecutableRunOptionsArgument(), builder(),
  ...
  if (PotentiallyImplementedAsEigenConvolution(*convolution,
    ...
    if (LayoutUtil::IsMonotonicWithDim0Major(lhs_shape.layout()) &&
      ...
      std::vector<llvm::Value*> args = {
          ...
          lhs_address,
          rhs_address,
          builder()->getInt64(input_batch),
      };
      for (int64_t d : input_dims) {
        args.push_back(builder()->getInt64(d));
      }
      args.push_back(builder()->getInt64(input_channels));
      for (int64_t d : kernel_dims) {
        args.push_back(builder()->getInt64(d));
      }
      args.push_back(builder()->getInt64(kernel_channels));
      args.push_back(builder()->getInt64(kernel_filters));
      ...
      for (const auto& p : padding) {
        args.push_back(builder()->getInt64(p.first));
        args.push_back(builder()->getInt64(p.second));
      }
      ...
      args.push_back(builder()->getInt64(convolution->feature_group_count()));

      VLOG(1) << "Ir emitter emitted Convolution to runtime:" << fn_name;
      EmitCallToFunc(fn_name, args, builder()->getVoidTy(),
                     /*does_not_throw=*/true,
...
absl::Status IrEmitter::HandleFft(HloInstruction* fft) {
  ...
  EmitCallToFunc(fn_name,
                 {GetExecutableRunOptionsArgument(), GetEmittedValueFor(fft),
                  operand_address, builder()->getInt32(fft->fft_type()),
                  builder()->getInt32(operand->shape().element_type() == F64 ||
                                      operand->shape().element_type() == C128),
                  builder()->getInt32(fft_rank), input_shape, fft_lengths},
                 builder()->getVoidTy(), /*does_not_throw=*/true,
...
absl::Status IrEmitter::HandleAllReduceSingleReplica(HloInstruction* crs) {
  ...
  llvm_ir::EmitTuple(GetIrArrayFor(crs), operand_ptrs, builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleAllReduceMultipleReplica(HloInstruction* crs) {
  ...
  llvm::Value* replica_groups_v =
      builder()->CreateGlobalStringPtr(replica_groups);
  ...
  llvm::Value* input_buffers = EncodeArrayFunctionArguments(
      input_buffer_ptrs, "input_buffers", builder());
  llvm::Value* output_buffers = EncodeArrayFunctionArguments(
      output_buffer_ptrs, "output_buffers", builder());

  int32_t shape_length;
  TF_ASSIGN_OR_RETURN(llvm::Value * shape_ptr,
                      llvm_ir::EncodeSelfDescribingShapeConstant(
                          crs->shape(), &shape_length, builder()));
  ...
  EmitCallToFunc(
      ...
      {/*run_options=*/GetExecutableRunOptionsArgument(),
       ...
       /*replica_groups_size=*/builder()->getInt32(replica_groups_size),

       /*channel_id_present=*/
       builder()->getInt32(static_cast<int32_t>(crs->channel_id().has_value())),
       /*use_global_device_ids=*/
       builder()->getInt32(static_cast<int32_t>(use_global_device_ids)),
       /*op_id=*/
       builder()->getInt64(crs->channel_id().has_value()
                               ? *crs->channel_id()
       ...
       /*shape_ptr=*/shape_ptr,
       /*shape_length=*/builder()->getInt32(shape_length),
       /*num_buffers=*/builder()->getInt32(crs->operand_count()),
...
absl::Status IrEmitter::HandleReduceScatter(HloInstruction* rs) {
  ...
  EmitCallToFunc(
      ...
      {/*run_options=*/GetExecutableRunOptionsArgument(),
       ...
       /*replica_groups_str_size=*/builder()->getInt32(replica_groups_size),

       /*channel_id_present=*/
       builder()->getInt32(static_cast<int32_t>(rs->channel_id().has_value())),
       /*use_global_device_ids=*/
       builder()->getInt32(static_cast<int32_t>(use_global_device_ids)),
       /*op_id=*/
       builder()->getInt64(rs->channel_id().has_value()
                               ? *rs->channel_id()
       ...
       /*element_type=*/
       builder()->getInt32(static_cast<int32_t>(datatype)),
       /*shape=*/builder()->getInt64(ShapeUtil::ElementsIn(rs->shape())),
...
absl::Status IrEmitter::HandleAllToAll(HloInstruction* instruction) {
  ...
  EmitCallToFunc(
      runtime::kAllToAllSymbolName,
      {
          ...
          /*op_id=*/
          builder()->getInt64(instruction->channel_id().has_value()
                                  ? *instruction->channel_id()
                                  : instruction->GetModule()->unique_id()),
          /*replica_groups=*/replica_groups_v,
          /*replica_groups_size=*/builder()->getInt32(replica_groups_size),
          /*num_buffers=*/builder()->getInt32(instruction->operand_count()),
          /*buffer_size=*/builder()->getInt64(buffer_size),
          /*source_buffers=*/input_buffers,
          /*destination_buffers=*/output_buffers,
      },
      builder()->getVoidTy());

  llvm_ir::EmitTuple(GetIrArrayFor(instruction), output_buffer_ptrs, builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleCollectivePermute(HloInstruction* crs) {
  ...
  llvm::Value* source_target_pairs_v =
      builder()->CreateGlobalStringPtr(source_target_pairs);
  ...
  EmitCallToFunc(
      ...
      {/*run_options=*/GetExecutableRunOptionsArgument(),
       ...
       builder()->getInt64(crs->channel_id().has_value()
                               ? *crs->channel_id()
                               : crs->GetModule()->unique_id()),
       /*byte_size=*/builder()->getInt32(ShapeUtil::ByteSizeOf(shape)),
       /*input_buffer=*/input_buffer,
       /*output_buffer=*/output_buffer,
       /*source_target_pairs=*/source_target_pairs_v,
       /*source_target_pairs_size=*/
       builder()->getInt32(source_target_pairs.size())},
      builder()->getVoidTy());
...
IrEmitter::EmitInnerLoopForVectorizedReduction(
  ...
  for (auto accumulator_shard_type : accumulator_type) {
    accumulator.push_back(llvm_ir::EmitAllocaAtFunctionEntry(
        accumulator_shard_type, "accumulator", builder(), 0));
  }
  ...
  llvm::Value* input_address =
      arg_array.EmitArrayElementAddress(input_index, builder());
  ...
  for (int i = 0; i < accumulator.size(); i++) {
    ...
    auto reduced_result =
        reduction_generator(builder(), current_accumulator_value, addend);
...
absl::StatusOr<bool> IrEmitter::EmitVectorizedReduce(
  ...
  llvm_ir::ForLoopNest loop_nest(IrName(reduce), builder());
  ...
  if (llvm::BasicBlock* innermost_body_bb =
    ...
    SetToFirstInsertPoint(innermost_body_bb, builder());
  }
  ...
  if (innermost_dimension_size >= vectorization_factor) {
    ...
    SetToFirstInsertPoint(loop->GetBodyBasicBlock(), builder());
    ...
    llvm::Value* output_address =
        target_array.EmitArrayElementAddress(array_index, builder());
    ...
    if (auto exit_terminator = loop->GetExitBasicBlock()->getTerminator()) {
      ...
      builder()->SetInsertPoint(exit_terminator);
    } else {
      CHECK_EQ(LayoutUtil::MinorToMajor(reduce->shape()).size(), 1);
      builder()->SetInsertPoint(loop->GetExitBasicBlock());
    }
  }
  ...
  if (innermost_dimension_size % vectorization_factor) {
    ...
    array_multi_index[innermost_dimension] =
        builder()->getInt64(innermost_dimension_size -
  ...
  if (outermost_loop_exit_block) {
    builder()->SetInsertPoint(outermost_loop_exit_block);
  }
...
absl::Status IrEmitter::HandleSlice(HloInstruction* slice) {
  ...
  llvm_ir::ForLoopNest loops(IrName(slice), builder());
  ...
  if (num_outer_loops > 0) {
    SetToFirstInsertPoint(loops.GetInnerLoopBodyBasicBlock(), builder());
  }
  ...
  if (num_outer_loops > 0) {
    SetToFirstInsertPoint(loops.GetOuterLoopExitBasicBlock(), builder());
  }
...
absl::Status IrEmitter::HandleDynamicUpdateSlice(
  ...
  if (ShapeUtil::IsScalar(dynamic_update_slice->shape())) {
    ...
    return llvm_ir::EmitDynamicUpdateSliceInPlace(
        ...
        IrName(dynamic_update_slice, "in_place"), builder());
  }
...
absl::Status IrEmitter::HandlePad(HloInstruction* pad) {
  ...
  llvm_ir::ForLoopNest loops(IrName(pad, "assign"), builder());
  ...
  for (size_t i = 0; i < operand_index.size(); ++i) {
    llvm::Value* offset =
        Mul(operand_index[i],
            builder()->getInt64(
                padding_config.dimensions(i).interior_padding() + 1));
    llvm::Value* index = Add(
        offset,
        builder()->getInt64(padding_config.dimensions(i).edge_padding_low()));
  ...
  output_array.EmitWriteArrayElement(output_index, operand_data, builder());
...
absl::Status IrEmitter::HandleFusion(HloInstruction* fusion) {
  ...
  if (llvm_ir::CanEmitFusedDynamicUpdateSliceInPlace(fusion, assignment_)) {
    ...
    return llvm_ir::EmitFusedDynamicUpdateSliceInPlace(
        fusion, GetIrArrayFor(fusion), &fused_emitter, builder());
    ...
    TF_RETURN_IF_ERROR(EmitDotOperation(
        *dot, target_array, lhs_array, rhs_array, &addend_array,
        GetExecutableRunOptionsArgument(), builder(), hlo_module_config_,
        target_machine_features_));
    return absl::OkStatus();
  } else {
...
absl::Status IrEmitter::HandleCall(HloInstruction* call) {
  ...
  if (backend_config_or.ok() &&
    ...
    TF_RETURN_IF_ERROR(EmitCallToParallelForkJoin(
        ...
        backend_config_or->outer_dimension_partitions(), builder(),
...
absl::Status IrEmitter::HandleSliceToDynamic(HloInstruction* hlo) {
  ...
  for (int64_t i = 1; i < hlo->operand_count(); ++i) {
    ...
    llvm::Value* metadata = builder()->CreateConstInBoundsGEP1_32(
        builder()->getInt8Ty(), dest_buffer,
        raw_data_size + dim_index * sizeof(int32_t));
    builder()->CreateStore(dyn_dim_size, metadata);
    dynamic_dims.push_back(
        builder()->CreateIntCast(dyn_dim_size, builder()->getInt64Ty(),
                                 /*isSigned=*/true, "i64_dyn_dim_size"));
  }
  ...
  auto loop_body_emitter =
    ...
    llvm::Value* source_element =
        GetIrArrayFor(hlo->operand(0))
            .EmitReadArrayElement(array_index, builder());
    llvm::Value* linear_index = array_index.Linearize(dynamic_dims, builder());
    // Delinearize the index based on the static shape.
    llvm_ir::IrArray::Index dest_index(linear_index, data_array.GetShape(),
                                       builder());
    data_array.EmitWriteArrayElement(dest_index, source_element, builder());
    return absl::OkStatus();
  };
  return llvm_ir::LoopEmitter(loop_body_emitter, data_array.GetShape(),
                              dynamic_dims, builder())
      .EmitLoop(IrName(hlo));
}
...
absl::Status IrEmitter::HandlePadToStatic(HloInstruction* hlo) {
  ...
  for (int i = 1; i < hlo->shape().tuple_shapes_size(); ++i) {
    ...
    const int64_t dim_index = i - 1;
    llvm::Value* metadata = builder()->CreateConstInBoundsGEP1_32(
        builder()->getInt8Ty(), source_buffer,
        raw_data_size + dim_index * sizeof(int32_t));
    llvm::Value* dyn_dim_size = builder()->CreateLoad(builder()->getInt32Ty(),
                                                      metadata, "dyn_dim_size");
    builder()->CreateStore(dyn_dim_size, dest_dim_size_address);
    dynamic_dims.push_back(
        builder()->CreateIntCast(dyn_dim_size, builder()->getInt64Ty(),
                                 /*isSigned=*/true, "i64_dyn_dim_size"));
  ...
  auto loop_body_emitter =
      [&](const llvm_ir::IrArray::Index& array_index) -> absl::Status {
    llvm::Value* linear_index = array_index.Linearize(dynamic_dims, builder());
    llvm_ir::IrArray::Index source_index(linear_index, input_shape, builder());
    llvm::Value* source_element =
        GetIrArrayFor(hlo->operand(0))
            .EmitReadArrayElement(source_index, builder());
    data_array.EmitWriteArrayElement(array_index, source_element, builder());
    return absl::OkStatus();
  };
  TF_RETURN_IF_ERROR(llvm_ir::LoopEmitter(loop_body_emitter, input_shape,
                                          dynamic_dims, builder())
                         .EmitLoop(IrName(hlo)));

  // Emit static tensor and dynamic sizes as one tuple.
  llvm_ir::EmitTuple(GetIrArrayFor(hlo), tuple_operand_ptrs, builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::HandleTopK(HloInstruction* hlo) {
  ...
  EmitCallToFunc(
      ...
      {builder()->getInt64(has_batch ? input->shape().dimensions(0) : 1),
       builder()->getInt64(input->shape().dimensions().back()),
       builder()->getInt64(k), values_ptr, out_values_ptr, out_indices_ptr},
      builder()->getVoidTy());
...
absl::Status IrEmitter::HandleOneDnnMatMulCalls(
  ...
  // Insert nargs.
  llvm::Value* nargs_val = builder()->getInt64(nargs);
  llvm::Value* nargs_ptr =
      llvm_ir::EmitAllocaAtFunctionEntry(i64_type, "nargs", builder());
  builder()->CreateLifetimeStart(nargs_ptr, builder()->getInt64(-1));
  builder()->CreateStore(nargs_val, nargs_ptr);
  args_val = builder()->CreateInsertValue(args_val, nargs_ptr, arg_indx++);

  // Insert ExecutableRunOptions.
  llvm::Value* run_opts_val = GetExecutableRunOptionsArgument();
  args_val = builder()->CreateInsertValue(args_val, run_opts_val, arg_indx++);
  ...
  llvm::Value* matmul_config_val =
      builder()->CreateGlobalStringPtr(llvm_ir::AsStringRef(str_config));
  args_val =
      builder()->CreateInsertValue(args_val, matmul_config_val, arg_indx++);
  ...
  absl::c_transform(custom_call->operands(), operands_stack_alloca.begin(),
                    [this](HloInstruction* instr) {
                      ...
                      return GetAllocaAndEmitMemrefInfo(*builder(), ir_array);
                    });
  for (int i = 0; i < num_operands; ++i) {
    args_val = builder()->CreateInsertValue(
        args_val, operands_stack_alloca[i].value, arg_indx++);
  }
  ...
  llvm::Value* args_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
      ptr_array_type, "matmul.args", builder());
  builder()->CreateLifetimeStart(args_ptr, builder()->getInt64(-1));
  builder()->CreateStore(args_val, args_ptr);
  ...
  if (use_scratchpad) {
    ...
    result_stack_alloca = GetAllocaAndEmitMemrefInfo(*builder(), result_array);
    ...
    scratch_stack_alloca =
        GetAllocaAndEmitMemrefInfo(*builder(), scratch_array);
    fn_call_args.push_back(scratch_stack_alloca.value);
    llvm_ir::EmitTuple(GetIrArrayFor(custom_call),
                       {result_slice_ptr, scratch_slice_ptr}, builder());
  } else {
    llvm_ir::IrArray result_array;
    result_array = GetIrArrayFor(custom_call);
    result_stack_alloca = GetAllocaAndEmitMemrefInfo(*builder(), result_array);
    fn_call_args.push_back(result_stack_alloca.value);
    fn_call_args.push_back(
  ...
  builder()->CreateLifetimeEnd(nargs_ptr, builder()->getInt64(-1));
  builder()->CreateLifetimeEnd(args_ptr, builder()->getInt64(-1));
...
absl::Status IrEmitter::HandleOneDnnConvolution(HloInstruction* custom_call) {
  ...
  llvm::Value* conv_config_val =
      builder()->CreateGlobalStringPtr(llvm_ir::AsStringRef(str_config));
  args_val =
      builder()->CreateInsertValue(args_val, conv_config_val, arg_indx++);
  ...
  llvm::Value* args_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
      ptr_array_type, "convolution.args", builder());
  builder()->CreateLifetimeStart(args_ptr, builder()->getInt64(-1));
  builder()->CreateStore(args_val, args_ptr);

  TF_RETURN_IF_ERROR(EmitTargetAddressForOp(custom_call));
  llvm_ir::IrArray result_array = GetIrArrayFor(custom_call);
  auto result_stack_alloca =
      GetAllocaAndEmitMemrefInfo(*builder(), result_array);

  EmitCallToFunc(runtime::kOneDnnConvolutionSymbolName,
                 {result_stack_alloca.value, args_ptr}, builder()->getVoidTy());
...
absl::Status IrEmitter::HandleOneDnnLayerNorm(HloInstruction* custom_call) {
  ...
  llvm::Value* ln_config_val =
      builder()->CreateGlobalStringPtr(llvm_ir::AsStringRef(str_config));
  args_val = builder()->CreateInsertValue(args_val, ln_config_val, arg_indx++);
  ...
  llvm::Value* args_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
      ptr_array_type, "layernorm.args", builder());
  builder()->CreateLifetimeStart(args_ptr, builder()->getInt64(-1));
  builder()->CreateStore(args_val, args_ptr);
...
absl::Status IrEmitter::HandleOneDnnSoftmax(HloInstruction* custom_call) {
  ...
  auto input_stack_alloca = GetAllocaAndEmitMemrefInfo(*builder(), input_array);
...
absl::Status IrEmitter::HandleCustomCall(HloInstruction* custom_call) {
  ...
  llvm::AllocaInst* operands_alloca =
      llvm_ir::EmitAllocaAtFunctionEntryWithCount(
          builder()->getPtrTy(), builder()->getInt32(operand_values.size()),
          "cc_operands_alloca", builder());
  if (emit_code_for_msan_) {
  ...
  if (custom_call->shape().IsTuple()) {
    ...
    llvm_ir::EmitTuple(GetIrArrayFor(custom_call), tuple_ptrs, builder());
  }
  ...
  switch (typed_custom_call->api_version()) {
    case CustomCallApiVersion::API_VERSION_ORIGINAL:
      EmitCallToFunc(custom_call->custom_call_target(),
                     {output_address, operands_alloca}, builder()->getVoidTy());
      break;
    ...
    case CustomCallApiVersion::API_VERSION_STATUS_RETURNING_UNIFIED: {
      ...
      EmitCallToFunc(
          ...
          {output_address, operands_alloca,
           builder()->CreateGlobalStringPtr(llvm_ir::AsStringRef(opaque)),
           builder()->getInt64(opaque.size()), GetStatusArgument()},
          builder()->getVoidTy());
      EmitEarlyReturnIfErrorStatus();
      break;
    }
    case CustomCallApiVersion::API_VERSION_TYPED_FFI: {
      ...
      llvm::AllocaInst* results_alloca =
          llvm_ir::EmitAllocaAtFunctionEntryWithCount(
              builder()->getPtrTy(), builder()->getInt32(buffer_ptrs.size()),
              "ffi_results_alloca", builder());
      if (emit_code_for_msan_) {
...
llvm::Value* IrEmitter::EmitPrintfToStderr(
  ...
  return builder()->CreateCall(
      builder()
          ...
          ->getParent()
          ->getParent()
          ->getOrInsertFunction(runtime::kPrintfToStderrSymbolName,
                                llvm::FunctionType::get(builder()->getInt32Ty(),
                                                        {builder()->getPtrTy()},
                                                        /*isVarArg=*/true)),
      call_args);
...
llvm::Value* IrEmitter::EmitCallToFfi(HloCustomCallInstruction* custom_call,
  ...
  EncodedInfo operand_types_encoded =
      StoreEncodedTypes("operands_types", operands, *builder());
  EncodedInfo operand_shapes_encoded =
      StoreEncodedShapes("operands_shapes", operands, *builder());

  EncodedInfo result_types_encoded =
      StoreEncodedTypes("results_types", result_shapes, *builder());
  EncodedInfo result_shapes_encoded =
      StoreEncodedShapes("results_shapes", result_shapes, *builder());
  ...
  std::vector<llvm::Value*> arguments = {
      ...
      /*target_name_ptr=*/builder()->CreateGlobalStringPtr(target_ref),
      /*target_name_len=*/builder()->getInt64(target.size()),
      /*outputs=*/results_alloca,
      /*inputs=*/operands_alloca,
      /*opaque_str_ptr=*/builder()->CreateGlobalStringPtr(opaque_ref),
      /*opaque_str_len=*/builder()->getInt64(opaque.size()),
      /*status_opaque=*/GetStatusArgument(),
      /*operand_types=*/operand_types_encoded.alloca,
      /*operand_count=*/builder()->getInt64(operand_types_encoded.size),
      /*operand_dims=*/operand_shapes_encoded.alloca,
      /*result_types=*/result_types_encoded.alloca,
      /*result_count=*/builder()->getInt64(result_types_encoded.size),
...
absl::Status IrEmitter::HandleConcatenate(HloInstruction* concatenate) {
  ...
  if (fast_impl_reason.ok()) {
    ...
    TF_RETURN_IF_ERROR(::xla::cpu::EmitFastConcatenate(
        concatenate, source_arrays, target_array, module_, *builder()));
...
absl::Status IrEmitter::HandleConditional(HloInstruction* conditional) {
  ...
  if (branch_index->shape().element_type() == PRED) {
    ...
    llvm_ir::LlvmIfData if_data =
        llvm_ir::EmitIfThenElse(pred_cond, "conditional", builder());

    SetToFirstInsertPoint(if_data.true_block, builder());
    EmitGlobalCall(*conditional->branch_computation(0),
                   IrName(conditional, "_true"));

    SetToFirstInsertPoint(if_data.false_block, builder());
    EmitGlobalCall(*conditional->branch_computation(1),
                   IrName(conditional, "_false"));

    SetToFirstInsertPoint(if_data.after_block, builder());
    return absl::OkStatus();
  }
  ...
  auto case_block = builder()->GetInsertBlock();
  llvm::BasicBlock* after_block;
  // Add a terminator to the case block, if necessary.
  if (case_block->getTerminator() == nullptr) {
    after_block = llvm_ir::CreateBasicBlock(nullptr, "case-after", builder());
    builder()->SetInsertPoint(case_block);
    builder()->CreateBr(after_block);
  } else {
    after_block =
        case_block->splitBasicBlock(builder()->GetInsertPoint(), "case-after");
  }
  ...
  auto default_block =
      llvm_ir::CreateBasicBlock(nullptr, "case-default", builder());
  ...
  builder()->SetInsertPoint(case_block);
  llvm::SwitchInst* case_inst = builder()->CreateSwitch(
      branch_index_value, default_block, num_branches - 1);
  ...
  for (int b = 0; b < num_branches - 1; ++b) {  // last branch is default
    ...
    auto branch_block = llvm_ir::CreateBasicBlock(
        nullptr, absl::StrCat("case-branch", b), builder());
    ...
    builder()->CreateBr(after_block);
    case_inst->addCase(builder()->getInt32(b), branch_block);
  }

  SetToFirstInsertPoint(after_block, builder());
  return absl::OkStatus();
}
...
absl::Status IrEmitter::FinishVisit(HloInstruction* root) {
  ...
  auto record_complete_computation = [&](llvm::Value* prof_counter) {
    if (prof_counter) {
      profiling_state_.RecordCompleteComputation(builder(), prof_counter);
    }
  };
...
llvm::Value* IrEmitter::GetProfileCounterCommon(
  ...
  return GEP(builder()->getInt64Ty(), GetProfileCountersArgument(),
             builder()->getInt64(prof_counter_idx), counter_name);
}
...
absl::Status IrEmitter::Preprocess(HloInstruction* hlo) {
  ...
  if (instruction_to_profile_idx_.count(hlo) ||
    ...
    tracing_state_.EmitTracingStart(builder(), hlo,
                                    GetExecutableRunOptionsArgument());
    profiling_state_.RecordCycleStart(builder(), hlo);
  }
  return absl::OkStatus();
}
...
absl::Status IrEmitter::Postprocess(HloInstruction* hlo) {
  if (auto* prof_counter = GetProfileCounterFor(*hlo)) {
    profiling_state_.RecordCycleDelta(builder(), hlo, prof_counter);
  }
  // When profiling is enabled, trace the same HLOs that the profiler does.
  if (instruction_to_profile_idx_.count(hlo) ||
      (hlo_module_config_.cpu_traceme_enabled() && !IsHloVeryCheap(hlo) &&
       hlo->parent()->IsEntryComputation())) {
    tracing_state_.EmitTracingEnd(builder(), hlo,
                                  GetExecutableRunOptionsArgument());
  }
  return absl::OkStatus();
}
...
void IrEmitter::EmitEarlyReturnIfErrorStatus() {
  ...
  llvm::Value* succeeded =
      EmitCallToFunc(runtime::kStatusIsSuccessSymbolName, {GetStatusArgument()},
                     builder()->getInt1Ty(), /*does_not_throw=*/true,
                     /*only_accesses_arg_memory=*/true);
  llvm_ir::EmitEarlyReturn(succeeded, builder(), GetReturnBlock());
}
  ...
  llvm::Value* tempbuf_address = [&]() -> llvm::Value* {
    ...
    if (param_it != computation_parameter_allocations_.end()) {
      ...
      llvm::Value* param_address_offset = llvm_ir::EmitBufferIndexingGEP(
          params, builder()->getPtrTy(), param_number, builder());
      llvm::LoadInst* param_address_untyped =
          Load(builder()->getPtrTy(), param_address_offset);
    ...
    if (buf_it == thread_local_buffers_.end()) {
      llvm::Value* buffer = llvm_ir::EmitAllocaAtFunctionEntry(
          ...
          builder(), MinimumAlignmentForShape(target_shape));
...
llvm::Value* IrEmitter::EmitGlobalBufferPointer(
  ...
  llvm::Value* tempbuf_address_ptr = llvm_ir::EmitBufferIndexingGEP(
      GetBufferTableArgument(), builder()->getPtrTy(), slice.index(),
      builder());
  llvm::LoadInst* tempbuf_address_base =
      Load(builder()->getPtrTy(), tempbuf_address_ptr);
  ...
  if (slice.offset() > 0) {
    ...
    tempbuf_address_untyped =
        InBoundsGEP(builder()->getInt8Ty(), tempbuf_address_base,
                    builder()->getInt64(slice.offset()));
  }
...
absl::Status IrEmitter::EmitTargetElementLoop(
  ...
  if (target_shape.IsTuple() &&
    ...
    TF_RETURN_IF_ERROR(
        llvm_ir::LoopEmitter(element_generator, output_arrays, builder())
    ...
    llvm_ir::EmitTuple(target_array, tuple_operand_ptrs, builder());
    ...
    if (ShouldEmitParallelLoopFor(*target_op)) {
      ...
      TF_RETURN_IF_ERROR(ParallelLoopEmitter(element_generator, target_array,
                                             &dynamic_loop_bounds, builder())
                             .EmitLoop(IrName(target_op)));
    } else {
      TF_RETURN_IF_ERROR(
          llvm_ir::LoopEmitter(element_generator, target_array, builder())
              .EmitLoop(IrName(target_op)));
    }
  }
  return absl::OkStatus();
...
absl::Status IrEmitter::DefaultAction(HloInstruction* hlo) {
  ...
  for (const HloInstruction* operand : hlo->operands()) {
    operand_to_generator[operand] = [=](const llvm_ir::IrArray::Index& index) {
      return GetIrArrayFor(operand).EmitReadArrayElement(index, builder());
    };
  }
  CpuElementalIrEmitter elemental_emitter(hlo_module_config_, this, module_);
...
std::vector<llvm::Value*> IrEmitter::EmitThreadLocalCall(
  ...
  llvm::AllocaInst* return_value_buffer = llvm_ir::EmitAllocaAtFunctionEntry(
      return_value_buffer_type, retval_alloca_name, builder(),
      retval_alignment);
...
void IrEmitter::EmitGlobalCall(const HloComputation& callee,
  ...
  Call(FindOrDie(emitted_functions_,
       ...
       GetArrayFunctionCallArguments(
           /*parameter_addresses=*/{}, builder(), name,
           /*return_value_buffer=*/
