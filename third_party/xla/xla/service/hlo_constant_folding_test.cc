...
TEST_F(HloConstantFoldingTest, FoldBroadcastWithSmallOperand) {
  const char* const kModuleStr = R"(
  HloModule test

  ENTRY r {
    a = f32[] constant(42)
    b = f32[2] constant({239, 239})
    broadcast_a = f32[2]{0} broadcast(a), dimensions={}
    ROOT cc = f32[2]{0} add(broadcast_a, b)
  })";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
