...
module attributes {"triton_gpu.num-warps" = 4 : i32} {
  ...
  tt.func @convert_sparse_local_load(%A: tensor<32x32xf16, #blocked>, %B: tensor<64x32xf16, #blocked>, %meta: tensor<32x4xi16, #blocked>) {
    // A_dot and B_dot local loads should not match with -sparse-local-load-to-llvm
    // CHECK-COUNT-2: triton_gpu.local_load
    %A_alloc = triton_gpu.local_alloc %A {allocation.offset = 0 : i32} : (tensor<32x32xf16, #blocked>) -> !tt.memdesc<32x32xf16, #shared, #triton_gpu.shared_memory>
    ...
    %meta_alloc = triton_gpu.local_alloc %meta {allocation.offset = 6144 : i32} : (tensor<32x4xi16, #blocked>) -> !tt.memdesc<32x4xi16, #shared, #triton_gpu.shared_memory>
    ...
    %D = triton_gpu.sparse_dot %A_dot, %B_dot, %acc, %meta_reg : tensor<32x32xf16, #dot_operand_a> meta tensor<32x4xi16, #dot_meta_enc> * tensor<64x32xf16, #dot_operand_b> -> tensor<32x32xf32, #mma>
    tt.return
  }
}
