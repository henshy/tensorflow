...
TEST(CollectiveDeviceListTest, DefaultListToProto) {
  CollectiveDeviceList list({{1, 2}, {3, 4}});
  EXPECT_THAT(list.ToProto(),
              testing::EqualsProto(CreateDeviceListProto({{1, 2}, {3, 4}})));
}
...
TEST(CollectiveDeviceListTest, DefaultListToProto2) {
  ...
  EXPECT_THAT(
      list.ToProto(),
      testing::EqualsProto(CreateDeviceListProto({{1, 2, 3, 4, 5, 6, 7}})));
}
...
TEST(CollectiveDeviceListTest, IotaListToProto) {
  CollectiveDeviceListProto expected_proto;
  IotaReplicaGroupListProto* iota_replica_group_list =
      expected_proto.mutable_iota_replica_group_list();
  ...
  CollectiveDeviceList list(IotaReplicaGroupList(2, 10));
  EXPECT_THAT(list.ToProto(), testing::EqualsProto(expected_proto));
}
